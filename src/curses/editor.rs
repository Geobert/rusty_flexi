...
pub fn process_key_up_down(cur_field: usize, up: bool, mut d: &mut FlexDay) {
    match cur_field {
        0 => { scroll_status(&mut d, up); },
        1 => { d.start = add_to_hour(d.start, up, 1); },
        2 => { d.start = add_to_minute(d.start, up, 1); }
        3 => { d.end = add_to_hour(d.end, up, 1); },
        4 => { d.end = add_to_minute(d.end, up, 1); },
...
pub fn process_digit_input_for_time(time: NaiveTime, field: TimeField,
        ...
        match digit {
            ...
            None => match field {
                TimeField::Hour => {
                    let t = time.hour() / 10;
                    time.with_hour(t)
                        .expect(&format!("something wrong while with_hour with {}", t))
                },
                TimeField::Minute => {
                    let t = time.minute() / 10;
                    time.with_minute(t)
                        .expect(&format!("something wrong while with_minute with {}", t))
                }
            }
        }
    } else {
...
pub fn process_digit_input_for_duration(duration: i64, field: TimeField,
...
pub fn process_digit_input(cur_field: usize, c: char, digit_idx: i32, mut d: &mut FlexDay) {
    match cur_field {
        0 => { unreachable!() },
        1 => {
            d.start =
                process_digit_input_for_time(d.start, TimeField::Hour, c, digit_idx);
        },
        2 => {
            d.start =
                process_digit_input_for_time(d.start, TimeField::Minute, c, digit_idx);
        }
        3 => {
        ...
        4 => {
            d.end =
                process_digit_input_for_time(d.end, TimeField::Minute, c, digit_idx);
        },
        5 => {
            d.pause =
                process_digit_input_for_duration(d.pause, TimeField::Hour, c, digit_idx);
        },
        6 => {
            d.pause =
                process_digit_input_for_duration(d.pause, TimeField::Minute, c, digit_idx);
        },
        _ => { unreachable!() },
    }
}